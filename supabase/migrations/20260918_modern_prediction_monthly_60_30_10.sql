...
begin
    ...
    month_events as (
        select value::integer as number, 3.5 * h.source_weight as score
        ...
        union all
        select value::integer as number, 0.8 * h.source_weight as score
        from target_history as h
        cross join lateral jsonb_array_elements_text(
    ...
    month_scores as (
        select number, sum(score) as score from month_events group by number
    ),
    recent_scores as (
    ...
    values (
        ...
        case when p_prediction_type = 'early'
            then 'monthly-60-recent-30-today-10-early'
            else 'monthly-60-recent-30-today-10'
        end,
    ...
    where public.prediction_snapshots.status = 'pending'
      and public.prediction_snapshots.engine_version is distinct from 'v22';
...
is 'Modern v22: 60% current-month target game, 30% latest seven target-game draws, 10% published today; reduced prior-month fallback when fewer than seven monthly draws exist.';
