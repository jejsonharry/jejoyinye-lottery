...
begin
    ...
    components as (
        select adaptive.*,
            (
                month_actual_norm * (
                    1.0 - month_classification_share - profile.moving_share
                )
            )
            + (month_classification_norm * month_classification_share)
            + (month_moving_norm * profile.moving_share)
            as month_component,
            (
                recent_actual_norm * (
                    0.95 - recent_classification_share - profile.moving_share
                )
            )
            + (recent_classification_norm * recent_classification_share)
            + (recent_moving_norm * profile.moving_share)
            + (pair_norm * 0.05)
            as recent_component,
            (
                today_actual_norm * (
                    0.95 - today_classification_share - profile.moving_share
                )
            )
            + (today_classification_norm * today_classification_share)
            + (today_moving_norm * profile.moving_share)
            + (carryover_norm * 0.05)
            as today_component,
            ...
            profile.recent_weight,
            profile.month_weight,
            profile.today_weight,
            profile.moving_share
        from adaptive
        cross join daily_profile_final as profile
    ),
    scored as (
        select components.*,
            (
            ...
            - least(5.0, feedback_penalty_norm * 0.05)
            as total_score,
            least(5.0, feedback_penalty_norm * 0.05)
    ...
    values (
        ...
        jsonb_build_object(
            'weeklyTargetGame', (
                select round(recent_weight, 4) from daily_profile_final
            ),
            'currentMonthSupport', (
                select round(month_weight, 4) from daily_profile_final
            ),
            'presentDayResults', (
                select round(today_weight, 4) from daily_profile_final
            ),
            ...
            'dailyPatternStrength', (
                select round(pattern_strength, 4) from daily_profile_final
            ),
            'dailyRepeatRate', (
                select round(repeat_rate, 4) from daily_profile_final
            ),
            'dailyMovingConfirmation', (
                select round(moving_confirmation, 4) from daily_profile_final
            ),
            'dailyClassificationConfirmation', (
                select round(classification_confirmation, 4) from daily_profile_final
            ),
            'dailyPredictionAverageHits', (
                select round(average_hits, 4) from daily_profile_final
            ),
            ...
            'movingNumbers', true,
            'movingShare', (
                select round(moving_share, 4) from daily_profile_final
            ),
            'pairRelationshipShare', 0.05,
            'previousGameCarryoverShare', 0.05,
...
is 'Modern v28 daily-adaptive engine: keeps the seven-day same-game core, raises or lowers live-day influence from completed draws, adapts moving support, applies classification conditionally, adds repeated-pair and previous-game carryover signals, and penalizes repeatedly missed prior picks.';
