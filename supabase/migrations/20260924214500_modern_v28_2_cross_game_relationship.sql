CREATE OR REPLACE FUNCTION prediction_private.modern_cross_game_score(p_target_order integer, p_draw_date date, p_number integer)
 RETURNS numeric
 LANGUAGE sql
 STABLE
 SET search_path TO ''
    ...
    select
        ...
        greatest(
            ...
            (
                greatest(
                    0,
                    (p_draw_date - r.draw_date) - 1
                )::numeric * 0.10
            )
        ) as recency_weight,
        case p_target_order - schedule.game_order
            when 1 then 1.00::numeric
            when 2 then 0.90::numeric
            when 3 then 0.80::numeric
            when 4 then 0.70::numeric
            when 5 then 0.62::numeric
            else 0.55::numeric
...
evidence as (
    select 3.0 * s.recency_weight * s.proximity_weight as score
    ...
    where value ~ '^([1-9]|[1-8][0-9]|90)$'
      and value::integer = p_number

    union all

    select 0.6 * s.recency_weight * s.proximity_weight
    from source_results as s
    cross join lateral jsonb_array_elements_text(
    ...
    select 1.1 * s.recency_weight * s.proximity_weight
    from source_results as s
    cross join lateral jsonb_array_elements_text(
        case when jsonb_typeof(s.winning) = 'array'
            then s.winning else '[]'::jsonb end
    ...
    select 0.8 * s.recency_weight * s.proximity_weight
    from source_results as s
    cross join lateral jsonb_array_elements_text(
        case when jsonb_typeof(s.winning) = 'array'
            then s.winning else '[]'::jsonb end
...
revoke execute on function prediction_private.modern_cross_game_score(integer,date,integer)
...
begin
    ...
    prediction_profile as (
        select
            ...
            case
                when p_prediction_type = 'early' then
                    least(
                        0.10::numeric,
                        greatest(0, game_order_value - 1)::numeric * 0.015
                    )
                else
                    least(
                        0.15::numeric,
                        greatest(0, game_order_value - 1)::numeric * 0.020
                    )
    ...
    raw_scores as (
        select number,
            ...
            prediction_private.modern_cross_game_score(
                game_order_value,
                p_draw_date,
                number
            ) as cross_game_score,
    ...
    values (
        ...
        case when p_prediction_type = 'early'
            then 'daily-adaptive-7day-cross-game-classification-moving-feedback-early'
            else 'daily-adaptive-7day-cross-game-classification-moving-feedback'
        end,
        ...
        jsonb_build_object(
            ...
            'crossGamePriorGames', greatest(0, game_order_value - 1),
            'crossGameShare', case
                when p_prediction_type = 'early' then
                    least(
                        0.10::numeric,
            ...
            'crossGameMethod', 'previous-scheduled-games-7day-recency-proximity-moving-classification',
            'engineRevision', 'v28.2-cross-game',
...
comment on function prediction_private.modern_cross_game_score(integer,date,integer)
is 'Scores one candidate number using the previous seven days of all earlier scheduled Modern games, weighted by recency and proximity to the target game, including movement and classification relationships.';

comment on function public.record_modern_prediction_snapshot(text,date,text)
is 'Modern v28.2 cross-game engine: preserves the target-game seven-day core and adds controlled cross-game evidence from all earlier scheduled games in the same seven-day window.';
