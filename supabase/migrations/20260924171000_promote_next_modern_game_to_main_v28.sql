...
begin
    ...
    where public.prediction_snapshots.status = 'pending'
      and (
          ...
          or public.prediction_snapshots.engine_profile is distinct from excluded.engine_profile
          or public.prediction_snapshots.weights->>'predictionType'
              is distinct from excluded.weights->>'predictionType'
...
declare
    ...
    changed boolean;
    first_remaining_order integer;
begin
    if p_draw_date is null then
        return 0;
    end if;

    first_remaining_order := greatest(coalesce(p_after_order, 0), 0) + 1;
...
is 'Promotes the immediate next Modern game to main/current prediction and keeps later games as early predictions; called after each completed result.';
