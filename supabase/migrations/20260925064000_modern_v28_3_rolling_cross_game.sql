CREATE OR REPLACE FUNCTION prediction_private.modern_cross_game_scores(p_target_order integer, p_draw_date date)
 RETURNS TABLE(number integer, score numeric, direct_winning_score numeric, machine_score numeric, moving_score numeric, classification_score numeric)
 LANGUAGE sql
 STABLE
 SET search_path TO ''
AS $function$
with source_results as (
    select
        ...
        r.machine,
        schedule.game_order,
        ((p_target_order - schedule.game_order + 12) % 12) as sequence_distance,
        greatest(
            0.40::numeric,
            1.00::numeric
            - greatest(0, (p_draw_date - r.draw_date) - 1)::numeric * 0.10
        ) as recency_weight,
        greatest(
            0.40::numeric,
            1.00::numeric
            - greatest(
                0,
                ((p_target_order - schedule.game_order + 12) % 12) - 1
              )::numeric * 0.06
        ) as proximity_weight
    from public.results as r
    cross join lateral (
        select case lower(trim(r.game))
    ...
    where r.lottery = 'modern-billionaire'
      and r.draw_date >= p_draw_date - 7
      and r.draw_date < p_draw_date
      and schedule.game_order is not null
      and schedule.game_order <> p_target_order
),
evidence as (
    select
        value::integer as number,
        3.0 * s.recency_weight * s.proximity_weight as direct_winning_score,
        0::numeric as machine_score,
        0::numeric as moving_score,
        0::numeric as classification_score
    from source_results as s
    cross join lateral jsonb_array_elements_text(
        case when jsonb_typeof(s.winning) = 'array'
            then s.winning else '[]'::jsonb end
    ) as win(value)
    ...
    select
        value::integer,
        0::numeric,
        0.6 * s.recency_weight * s.proximity_weight,
        0::numeric,
        0::numeric
    from source_results as s
    cross join lateral jsonb_array_elements_text(
        case when jsonb_typeof(s.machine) = 'array'
            then s.machine else '[]'::jsonb end
    ) as machine(value)
    where value ~ '^([1-9]|[1-8][0-9]|90)$'
    ...
    select
        ...
        0::numeric,
        0::numeric,
        1.1 * s.recency_weight * s.proximity_weight,
        0::numeric
    from source_results as s
    cross join lateral jsonb_array_elements_text(
        case when jsonb_typeof(s.winning) = 'array'
    ...
    select
        ...
        0::numeric,
        0::numeric,
        0.8 * s.recency_weight * s.proximity_weight
    from source_results as s
    cross join lateral jsonb_array_elements_text(
        case when jsonb_typeof(s.winning) = 'array'
            then s.winning else '[]'::jsonb end
...
select
    evidence.number,
    sum(
        direct_winning_score
        + machine_score
        + moving_score
        + classification_score
    )::numeric as score,
    sum(direct_winning_score)::numeric,
    sum(machine_score)::numeric,
    sum(moving_score)::numeric,
    sum(classification_score)::numeric
from evidence
where evidence.number between 1 and 90
group by evidence.number;
$function$


revoke execute on function prediction_private.modern_cross_game_scores(integer,date)
...
begin
    ...
    prediction_profile as (
        select
            ...
            end as effective_cross_game_share
    ...
    cross_game_scores as (
        select *
        from prediction_private.modern_cross_game_scores(
            game_order_value,
            p_draw_date
        )
    ),
    raw_scores as (
        select number,
            ...
            coalesce(cross_game_scores.score, 0) as cross_game_score,
            coalesce(cross_game_scores.direct_winning_score, 0) as cross_game_direct_score,
            coalesce(cross_game_scores.machine_score, 0) as cross_game_machine_score,
            coalesce(cross_game_scores.moving_score, 0) as cross_game_moving_score,
            coalesce(cross_game_scores.classification_score, 0) as cross_game_classification_score,
        ...
        left join cross_game_scores using (number)
    ...
    max_scores as (
        select max(month_score) as month_max,
            ...
            max(cross_game_score) as cross_game_max,
    ...
    normalized as (
        select raw_scores.*,
            ...
            coalesce((cross_game_score / nullif(cross_game_max, 0)) * 100, 0) as cross_game_norm,
    ...
    components as (
        select adaptive.*,
            ...
            profile.effective_cross_game_share as cross_game_share
        from adaptive
    ...
    scored as (
        select components.*,
            (
                recent_component * greatest(
                    0,
                    recent_weight - cross_game_share
                )
                + cross_game_norm * cross_game_share
    ...
    select
        ...
        jsonb_agg(
            jsonb_build_object(
                ...
                'crossGameScore', round(cross_game_score, 4),
                'crossGameDirectScore', round(cross_game_direct_score, 4),
                'crossGameMachineScore', round(cross_game_machine_score, 4),
                'crossGameMovingScore', round(cross_game_moving_score, 4),
                'crossGameClassificationScore', round(cross_game_classification_score, 4),
                'crossGameNormalized', round(cross_game_norm, 4),
                'crossGameShare', round(cross_game_share, 4),
    ...
    values (
        ...
        case when p_prediction_type = 'early'
            then 'daily-adaptive-7day-rolling-cross-game-classification-moving-feedback-early'
            else 'daily-adaptive-7day-rolling-cross-game-classification-moving-feedback'
        end,
        ...
        jsonb_build_object(
            ...
            'crossGameRelationship', true,
            'crossGameWindowDays', 7,
            'crossGameHistoricalOtherGames', 11,
            'crossGameShare', case
                when p_prediction_type = 'early' then 0.10
                else 0.15
            end,
            'crossGameMethod', 'rolling-7day-all-other-games-cyclic-proximity-recency-moving-classification',
            'crossGameSequenceMode', 'continuous-across-day-boundaries',
            'engineRevision', 'v28.3-rolling-cross-game',
...
comment on function prediction_private.modern_cross_game_scores(integer,date)
is 'v28.3 rolling cross-game scorer: all other Modern games from the previous seven completed days, weighted by recency and cyclic schedule proximity.';

comment on function public.record_modern_prediction_snapshot(text,date,text)
is 'Modern v28.3 rolling cross-game engine with same-game 7-day core, all-other-games rolling cross-game history, live-day adaptation, moving/classification/pair/carryover and feedback.';
